 ...
 * @author hanli
 * @deprecated Please use the QLearningSim class via the gui.
 */
@Deprecated
...
//	private double epsilon = 0.1;
	...
	public void run() {
		...
		int lightDelay = 3;
		State prevState = sensor.getState(trafficModel, trafficLight, lightDelay, stateType);
		...
		for(int count=0; count<timesteps; count++) {			
			...
			if(lightDelay > 0) {
				...
				State currState = sensor.getState(trafficModel, trafficLight, lightDelay, stateType);
	...
	private int nextStep() {
        ModelAndLight env = simulator.processStep(trafficModel, trafficLight);
		trafficModel = env.getTfModel();
